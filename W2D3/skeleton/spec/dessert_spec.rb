...
describe Dessert do
  let(:chef) { double("chef", name: "Guy Fieri") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect{cake.quantity('lol')}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Guy Fieri")
      expect(cake.serve).to eq("Guy Fieri has made 10 cakes!")
    end
  end
